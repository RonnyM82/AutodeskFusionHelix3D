...
You give it a table of stations. Station 1 is where the helix starts. Each row carries a pitch and a radius, and each row after the first says how many turns on from the row above it sits. Pitch is the rise per turn, so the height is not something you type: it falls out of the table, and the dialog tells you what it came to. Click into a row and three things appear on the preview: an orange cross on that station, the run its turns value measures lit up behind it, and a note beside it giving the turns, the rise over them, and the pitch and radius at that station. The note faces you whichever way the model is spun. Row 1 gets the cross and the note but no lit run, because it is where the helix starts and has nothing before it, which is what its turns cell is telling you when it says start.

**Pin the height** turns that round. Normally you chase a height by trying pitches until the readout says what you wanted. Tick this on instead, say how tall the helix has to be, and tick the station in the **Sets height** column that gives way. Its pitch stops being something you type and is worked out for you, so the helix comes out at the height you asked for. Every other station keeps exactly the pitch you typed, which is the point: the closed ends of a spring, or the lead either side of a dwell, stay where you put them while the middle takes up the difference.

Any station next to the ticked one carrying the same pitch is worked out along with it, to the same value. A run of two stations at one pitch is how you say "constant along here", and solving them separately would put a kink in the middle of it. So they move together and the run stays constant.

**Height along** is the other half of it. Pick a sketch line or a straight edge, or pick two points, and the distance between them is the height. The helix runs along them as well, so that geometry sets the axis and the position too, and the **Plane**, **Center Point** and **Placement** triad all step aside while it is set. Move the line and the helix follows: new length, new solved pitch, same everything else. A **Start Point** still works and still sets the angle the first coil begins at. **Flip direction** starts the helix at the far end of the line and builds back down it.
...
- On a variable pitch helix, **Lock radius to start point** appears under the Start Point picker as soon as you pick one, and starts out ticked. While it is ticked the radius in row 1 is measured from the point rather than typed, so the cell greys out, and it keeps up on its own: drag the point later and the helix follows, the same way the constant pitch one does. Untick it and the point sets the angle and the height but leaves the radius alone.
- It never touches the rest of the Radius column, so a helix that tapers keeps its taper. **Copy radius to every station**, under the table, fills the column from row 1 when the whole helix is one radius. Pin the height to a line and the start point sets the angle and that first radius only, since the line is already saying where the helix begins and how tall it is.
- Variable pitch helices made before 0.9.0 open with the lock off, so their row 1 stays exactly as it was typed.
- Every variable pitch helix carries a **Height** row in the Parameters dialog whether or not the height is pinned. It has to: Fusion settles which parameters a feature owns the moment it is created, so one built without it could never be pinned later. Left unpinned it is only telling you what the table came to, and editing it there does nothing.
- Variable pitch helices made before 0.9.0 have no Height parameter, so the pin is greyed out when you edit one. Pinning the height on one of those means making a new helix.
- When the height is pinned, the pitch of the stations that gave way belongs to the add-in. Change it in the command's dialog, not in the Parameters dialog: editing it there breaks the run into pieces and the constant stretch stops being constant.
- A helix pinned to a line keeps its curve right when the line moves, but the pitch rows in the Parameters dialog still read what they did the last time the dialog was open. Open the helix for editing and they catch up.
